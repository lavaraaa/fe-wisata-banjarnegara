...
const UserSidebar = () => {
  ...
  const openLoginModal = () => {
    ...
    const handleHidden = () => {
      setShowLogin(true);
      sidebar.removeEventListener('hidden.bs.offcanvas', handleHidden);
    };
    sidebar.addEventListener('hidden.bs.offcanvas', handleHidden);
    closeSidebar();
  };
  ...
  return (
    <>
      ...
      >
        <div className="offcanvas-header d-flex flex-column align-items-start" style={{ background: '#015E78' }}>
          <div className="d-flex justify-content-between w-100 align-items-center">
            ...
            <h6 className="offcanvas-title text-white" id="sidebarMenuLabel">
              WisataBanjarnegara
            </h6>
          ...
          {user && (
            <div
              className="mt-3 d-flex flex-column align-items-start w-100"
            ...
            {!user ? (
              <>
                <li className="nav-item mt-3">
                  <button onClick={openRegisterModal} className="btn btn-outline-secondary w-100 mb-2">
                    Daftar
                  </button>
                </li>
                <li className="nav-item">
                  <button onClick={openLoginModal} className="btn w-100" style={{ backgroundColor: '#015E78', color: 'white' }}>
                    Masuk
                  </button>
                </li>
              </>
      ...
      {/* MODAL AUTH */}
      {showLogin && <Login show={showLogin} onClose={() => setShowLogin(false)} toRegister={openRegisterModal} />}
      {showRegister && <Register show={showRegister} onClose={() => setShowRegister(false)} toLogin={openLoginModal} />}
